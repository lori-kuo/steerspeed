...
class UdacityDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        :return:
        """
        path_image_steer_speed, path_label_speed_steer = self.img_info[index]

        image_use = []
        steer_use = []
        speed_use = []
        for i in range(len(path_image_steer_speed)):
            img = Image.open(path_image_steer_speed[i][0]).convert('RGB')
            if self.transform is not None:
                img = self.transform(img)
            image_use.append(img)
            #   print("speed_steer_test_path", path_image_speed[i])
            with open(path_image_steer_speed[i][1], encoding='utf-8') as file:
                content = file.read()
                content.rstrip()
            steer_use.append(float(content))

            with open(path_image_steer_speed[i][2], encoding='utf-8') as file:
                content = file.read()
                content.rstrip()
        ...
        return image_use, steer_use, speed_use, speed_label, steer_label
    ...
    def _get_img_info(self):
        ...
        for i in range(len(os.listdir(os.path.join(self.root_dir)))):
            ...
            image_steer_speed_use = []
        
            for i in range(len(path_images) - 39):  # 最后必须保留40个样本
                image_steer_speed_use_item = []
                for j in range(20):  # 输入前20帧
                    image_steer_speed = (path_images[i + j], path_steers[i + j], path_speeds[i+j])
                    image_steer_speed_use_item.append(image_steer_speed)
                image_steer_speed_use.append(image_steer_speed_use_item)
            ...
            for i in range(20, len(path_images) - 19):  # 最后必须保留20个样本
                ...
                for j in range(20):  # 预测20帧
                    image_steer_speed = (path_speeds[i + j], path_steers[i + j], path_speeds[i+j])
                    speed_steer_label_item.append(image_steer_speed)
                speed_steer_label.append(speed_steer_label_item)
            img_info = [(p, idx) for p, idx in zip(image_steer_speed_use, speed_steer_label)]
...
if __name__ == "__main__":
    root_dir = r"E:/MyProgram/python/stersped_data/train"
